    ...
    try {
      ...
      try {
        ...
        if (teamsError instanceof Error && teamsError.message.includes('404')) {
          setError('Database migration required. Please run: npx supabase db reset');
          return;
        }
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.message.includes('404') || err.message.includes('does not exist')) {
          errorMessage = 'Database migration required. Please run: npx supabase db reset';
        } else {
              ...
              {error.includes('migration required') && (
                <div className="text-slate-400 text-xs space-y-1">
                  ...
                  <ol className="list-decimal list-inside space-y-1 ml-2">
                    ...
                    <li>Run: <code className="bg-slate-800 px-1 rounded">npx supabase db reset</code></li>
